...
class Cart {
    constructor(_id, _name, _category, _price, _amount, _index){
        this.id = _id
        this.name = _name
        this.category = _category
        this.price = _price
...
const addToCart = (data) => {
    ...
    let addId = listProduct[data].id
    let addName = listProduct[data].name
    let addCategory = listProduct[data].category
    let addPrice = listProduct[data].price
    let cartProduct = new Cart(addId, addName, addCategory, addPrice, 1, data)
...
const showCart = () => {
    ...
    listCart.forEach((value, index) => {
        listToPay += `<tr>
                        ...
                        <td>${value.name}</td>
                        <td>${value.price}</td>
                        <td><input type='button' onclick='deleteData(${index}, "cart", ${value.index})' value='Delete'></td>
                    </tr>`
    })
...
const pay = () => {
    ...
    listCart.forEach((value) => {
        ...
        totalToPay += parseInt(value.price)
        detailsToPay += `<p>${value.id} | ${value.category} | ${value.name} | ${value.price}</p>`
    })
...
const deleteData = (data, stat, ind) => {
    if(stat == 'product'){
        listProduct.splice(data, 1)
        showData()
    }else{
        console.log(data)
        listCart.splice(data, 1)
        listProduct[ind].stock += 1
        showCart()
        showData()
    }
}
